...
export async function getAppStats(appId: string, now: Date): Promise<AppStats> {
  ...
  const [
    ...
    prisma.$queryRaw<{ c: bigint }[]>`
      SELECT COUNT(DISTINCT uid) AS c FROM events
      WHERE "appId" = ${appId} AND uid IS NOT NULL`,
    prisma.session.count({ where: { appId } }),
  ]);

  const totalUsers = Number(totalUsersRows[0]?.c ?? 0);
  // Events with a uid only — anonymous (uid-less) events excluded from per-user avg.
  const eventsWithUid = await prisma.event.count({
    where: { appId, uid: { not: null } },
  });
...
export async function listAppGeo(appId: string): Promise<AppGeo> {
  const [countries, languages, baseRows] = await Promise.all([
    prisma.$queryRaw<{ code: string; users: bigint; events: bigint }[]>`
      ...
      FROM events
      WHERE "appId" = ${appId} AND props->>'country' IS NOT NULL
      GROUP BY 1 ORDER BY users DESC, events DESC LIMIT 12`,
    prisma.$queryRaw<{ code: string; users: bigint; events: bigint }[]>`
      SELECT lower(props->>'lang') AS code,
             COUNT(DISTINCT uid) AS users, COUNT(*) AS events
      FROM events
      WHERE "appId" = ${appId} AND props->>'lang' IS NOT NULL
      GROUP BY 1 ORDER BY users DESC, events DESC LIMIT 8`,
    prisma.$queryRaw<{ c: bigint }[]>`
      SELECT COUNT(DISTINCT uid) AS c FROM events
      WHERE "appId" = ${appId} AND props->>'country' IS NOT NULL AND uid IS NOT NULL`,
  ]);
